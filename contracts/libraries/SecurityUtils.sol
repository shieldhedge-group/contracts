...
/// @title SecurityUtils
/// @notice Library for security validation functions to reduce contract size
library SecurityUtils {
    ...
    /// @dev SECURITY FIX: Increased gas limit to 10000 to support multisigs and proxy contracts
    function secureETHTransfer(address payable to, uint256 amount, address whitelistManager) internal {
        if (to.code.length == 0) {
            ...
            // Increased from 5000 to 10000 gas to support complex contracts (multisigs, proxies)
            // Still low enough to prevent reentrancy attacks
